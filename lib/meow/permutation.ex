defmodule Meow.Permutation do
  ...
  @doc """
  Generates populations, where each genome is a random permutation of
  numbers from `0` to `length - 1`.
  ...
  defn init_random(opts \\ []) do
    ...
    Nx.random_uniform({pop_count, pop_size, length})
    |> Nx.argsort(axis: 2)
    |> Nx.as_type({:u, 16})
  end
  ...
  defn crossover_order(genomes) do
    {pop_count, pop_size, _length} = Nx.shape(genomes)
    ...
    |> shift(Nx.reshape(offset, {pop_count, pop_size}), axis: 2)
  end
  ...
  defn crossover_position_based(genomes) do
    ...
    fix_position? =
      Nx.random_uniform({pop_count, half_pop_size, length}, 0, 2)
      |> duplicate_along_axis(axis: 1)

    split_position = Nx.sum(fix_position?, axes: [2], keep_axes: true)
  ...
  defn mutation_inversion(genomes, opts \\ []) do
    ...
    mutated = Nx.take_along_axis(genomes, mapping, axis: 2)
  ...
  defn mutation_swap(genomes, opts \\ []) do
    ...
    swap_position1 = Nx.random_uniform({pop_count, pop_size}, 0, length)
    swap_position2 = Nx.random_uniform({pop_count, pop_size}, 0, length - 1)
    ...
    indices =
      Nx.stack(
        [
          Nx.iota({pop_count, pop_size, 2}, axis: 0),
          Nx.iota({pop_count, pop_size, 2}, axis: 1),
          swap_positions
        ],
        axis: -1
      )
    ...
    swapped_values = Nx.reverse(values, axes: [2])
    diff = swapped_values - values

    mutated =
      Nx.indexed_add(
        genomes,
        Nx.reshape(indices, {:auto, 3}),
  ...
  defnp incorporate_mutated(genomes, mutated_genomes, probability) do
    ...
    mutate? = Nx.random_uniform({pop_count, pop_size, 1}) |> Nx.less(probability)

    mutate?
    |> Nx.broadcast(genomes)
    |> Nx.select(mutated_genomes, genomes)
  end

  defnp positions_single_point(positions, parent_positions, split_position) do
    {_pop_count, _pop_size, length} = Nx.shape(positions)
    ...
    Nx.select(
      ...
      parent_positions,
      parent_positions |> swap_pairs_along_axis(axis: 1) |> Nx.add(length)
    )
    |> relative_positions_to_permutations()
  end
  ...
  defnp mapped_single_point(genomes, mapping, split_position) do
    ...
    mapped_genomes = Nx.take_along_axis(genomes, mapping, axis: 2)
    ...
    |> Nx.take_along_axis(reverse_mapping, axis: 2)
  end
  ...
  defnp random_genome_blocks(genomes, opts \\ []) do
    ...
    {offset, block_length} =
      transform({opts[:paired], pop_count, pop_size, length}, fn
        {false, pop_count, pop_size, length} ->
          random_blocks(n: pop_count * pop_size, length: length)

        {true, pop_count, pop_size, length} ->
          {offset, block_length} = random_blocks(n: pop_count * div(pop_size, 2), length: length)
          {duplicate_along_axis(offset), duplicate_along_axis(block_length)}
      end)

    shape = {pop_count, pop_size, 1}
    type = Nx.type(genomes)

    {offset |> Nx.reshape(shape) |> Nx.as_type(type),
     block_length |> Nx.reshape(shape) |> Nx.as_type(type)}
  end
  ...
  defnp random_blocks(opts \\ []) do
    ...
    idx1 = Nx.random_uniform({n}, 0, length)
    idx2 = Nx.random_uniform({n}, 0, length)
  ...
  defnp permutations_to_positions(permutations) do
    permutation_argsort(permutations, axis: 2)
  end
